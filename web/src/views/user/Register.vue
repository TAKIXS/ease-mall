...
async function handleRegister() {
  ...
  try { await auth.register(form); ElMessage.success('注册成功，欢迎加入~'); router.push('/login') } catch (e) {}
}
</script>
    ...
    <div class="card">
      <div class="card-art">
        <svg viewBox="0 0 120 80" fill="none">
          <circle cx="60" cy="30" r="18" fill="#F0E6D8" stroke="#C4A484" stroke-width="2"/>
          <path d="M38 55 Q60 35 82 55" fill="#F0E6D8" stroke="#C4A484" stroke-width="2"/>
          <circle cx="52" cy="28" r="2" fill="#8B5E3C"/>
          <circle cx="68" cy="28" r="2" fill="#8B5E3C"/>
        </svg>
        <h2>创建账号</h2>
        <p>加入 ease-mall，发现生活好物</p>
      </div>

      <el-form :model="form" size="large" label-position="top">
        <el-form-item label="用户名"><el-input v-model="form.username" placeholder="3-20位" class="warm-input" /></el-form-item>
        <el-form-item label="密码"><el-input v-model="form.password" type="password" placeholder="6-30位" show-password class="warm-input" /></el-form-item>
        <el-form-item label="手机号"><el-input v-model="form.phone" placeholder="选填" class="warm-input" /></el-form-item>
        <el-form-item label="邮箱"><el-input v-model="form.email" placeholder="选填" class="warm-input" /></el-form-item>
        <el-form-item><el-button class="warm-btn" @click="handleRegister">注 册</el-button></el-form-item>
      </el-form>

      <p class="link-text">已有账号？<a @click="router.push('/login')">去登录</a></p>
    </div>
  </div>
</template>
...
.card { width: 420px; background: var(--card); border-radius: var(--radius); box-shadow: var(--shadow-lg); padding: 44px 40px 36px; }
