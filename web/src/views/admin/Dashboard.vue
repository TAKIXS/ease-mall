...
import { User, ShoppingCartFull, TrendCharts, Van, Files, Money, Coin } from '@element-plus/icons-vue'
...
const cards = [
  {label:'总用户数',val:()=>dashboard.value.totalUsers||0,icon:User,bg:'#F5EDE4',accent:'#8B5E3C'},
  {label:'总订单数',val:()=>dashboard.value.totalOrders||0,icon:ShoppingCartFull,bg:'#EDF0E8',accent:'#5B9A8B'},
  {label:'今日订单',val:()=>dashboard.value.todayOrders||0,icon:TrendCharts,bg:'#E8EEF5',accent:'#6B8DB5'},
  {label:'待发货',val:()=>dashboard.value.pendingShip||0,icon:Van,bg:'#F5E8E8',accent:'#C48B8B'},
  {label:'商品总数',val:()=>dashboard.value.totalProducts||0,icon:Files,bg:'#EEE8F0',accent:'#8B6B9E'},
  {label:'总交易额',val:()=>'¥'+(dashboard.value.totalGMV||0),icon:Money,bg:'#F5F0E8',accent:'#B8954A'},
  {label:'今日交易额',val:()=>'¥'+(dashboard.value.todayGMV||0),icon:Coin,bg:'#E8F0F0',accent:'#5B8B8B'},
]
</script>
  ...
  <div class="dash-wrap">
    ...
    <div class="stat-grid">
      <div v-for="c in cards" :key="c.label" class="stat-card" :style="{background:c.bg}">
        <el-icon :size="28" :color="c.accent"><component :is="c.icon" /></el-icon>
        <span class="stat-num" :style="{color:c.accent}">{{ c.val() }}</span>
        <span class="stat-label">{{ c.label }}</span>
      </div>
    </div>

    <h3 style="margin:28px 0 14px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:8px">
      <el-icon :size="20"><User /></el-icon> 用户管理
    </h3>
...
.stat-card { border-radius: 16px; padding: 22px; display: flex; flex-direction: column; gap: 8px; transition: transform .2s; }
